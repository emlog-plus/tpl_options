$(function() {
  ...
  var message = $('<div/>').appendTo($('#container .container-fluid:first'));
  ...
  (function() {
    ...
    if (tplOptions.templates[currentTemplate]) {
      $('.theme-used').insertAfter($('<div class="theme-actions"><a class="btn btn-primary">设置</a></div>').appendTo($('.theme_ntpls ')).addClass(attr('setting')).data('template', currentTemplate));
    }
  })();
  //绑定事件
  ...
  function showMsg(code, msg) {
    ...
    if (code == 0) {
      message.attr('class', 'alert alert-info alert-dismissable');
      if (timer) {
